    ...
    Attrs: OpAttributes<Transaction = TxTy<N::Types>, RpcPayloadAttributes: DeserializeOwned>,
{
    ...
    async fn launch_add_ons(
        ...
        // Store SGT config for later use
        ...
        rpc_add_ons
            .launch_add_ons_with(ctx, move |container| {
                ...
                if let Some((sgt_addr, sgt_port)) = sgt_config {
                    ...
                    // Get the eth_api from the registry
                    let standard_eth_api = registry.eth_api();

                    // Create SGT-enabled API using the EthApiWithSgtMode trait
                    // This creates a new API instance that shares underlying components
                    // (provider, pool, etc.) but has an independent SGT mode flag
                    let sgt_eth_api = standard_eth_api.clone_with_sgt_mode(true);

                    // Create RPC module with eth_* methods
                    let mut module = RpcModule::new(());
                    module.merge(sgt_eth_api.into_rpc())
                        .map_err(|e| eyre::eyre!("Failed to merge SGT eth RPC methods: {}", e))?;

                    // Start HTTP server in background task
                    // Note: We use tokio::spawn because we're inside the launch_add_ons_with closure
                    // which doesn't have direct access to ctx.node.task_executor().
                    // This is acceptable for this non-critical background task.
                    ...
                    tokio::spawn(async move {
                        match jsonrpsee::server::ServerBuilder::default().build(socket).await {
                            Ok(server) => {
                                ...
                                // Keep the server running by awaiting the stopped signal
...
impl<NetworkT, RpcMiddleware> OpAddOnsBuilder<NetworkT, RpcMiddleware> {
    ...
    /// Configure SGT HTTP server address and port
